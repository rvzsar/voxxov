...
**Готовый бинарник** — скачайте `voxxov-<версия>-windows-x86_64.exe` со страницы [Releases](https://github.com/rvzsar/voxxov/releases):
- версионные сборки (теги `v*`) — стабильные;
- `rolling` — свежая сборка из `main`, пересобирается при каждом push.

Положите `.exe` в любую доступную для записи папку и запустите — установщик
и права администратора не нужны.

При первом запуске скачиваются yt-dlp/ffmpeg и модель (~330 МБ) в `bin/` и
`models/` рядом с `.exe`. Дальше приложение работает офлайн.
...
Ставит npm/pnpm-зависимости и запускает `tauri dev`.
